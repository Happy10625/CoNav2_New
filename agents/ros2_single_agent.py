...
class ROS_Agent(VLM_Agent):
    ...
    def mapping(self, observations):
        ...
        if self.args.visualize:
            Open3d_goal_pose = []
            cam_k = (float(self.camera_K.fx), float(self.camera_K.fy),
                     float(self.camera_K.cx), float(self.camera_K.cy))
    ...
    def _visualize(self, map_pred, exp_pred, map_edge, goal_map, text_queries):
        ...
        # NOTE: Only resize for visualization. Mapping/navigation still uses
        # the original sensor resolution.
        annotated_vis = self.annotated_image
        if annotated_vis is not None:
            annotated_vis = cv2.resize(annotated_vis, (640, 480), interpolation=cv2.INTER_AREA)
        vis_image_rgb[50:530, 15:655] = annotated_vis
