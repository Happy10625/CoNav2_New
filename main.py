...
def main(args, send_queue, receive_queue):
    ...
    valid_episodes = [ep for ep in dataset.episodes if "00800" in ep.scene_id]
    ...
    while count_episodes < num_episodes:
        ...
        for i in range(num_agents):
            # 修复1：兼容单/多智能体观测值，删除重复调用
            obs = observations[i] if num_agents > 1 else observations
            agent[i].reset(obs, agent_state)
            actions.append(0)
        ...
        while not env.episode_over:
            ...
            for i in range(num_agents):
                ...
                # 修复2：兼容单/多智能体观测值
                obs = observations[i] if num_agents > 1 else observations
                agent[i].mapping(obs, agent_state)
