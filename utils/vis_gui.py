    ...
    def update_main(self):
        ...
        while not self.is_done:
            if not self.receive_queue.empty():
                payload = self.receive_queue.get()

                # Backward-compatible payload formats:
                # - Old:  [agent_id, image_rgb, image_depth, annotated_image, vis_image, ...]
                # - New:  [agent_id, image_rgb, image_depth, cam_k, annotated_image, vis_image, ...]
                if len(payload) == 12:
                    (agent_id, image_rgb, image_depth,
                     annotated_image, vis_image,
                     point_sum_points, point_sum_colors,
                     traj, episode_n, plan_path, Open3d_goal_pose, time_step_info) = payload
                    cam_k = None
                elif len(payload) == 13:
                    (agent_id, image_rgb, image_depth,
                     cam_k,
                    ...
                    raise ValueError(f"Unexpected GUI payload length: {len(payload)}")
                ...
                # Build intrinsics per frame using real image size.
